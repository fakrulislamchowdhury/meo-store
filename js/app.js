...
const showProducts = (products) => {
    ...
    for (const product of allProducts) {
        ...
        <button onclick="loadProduct('${product.title}, ${product.price}, ${product.rating.count}, ${product.rating.rate}')" id="details-btn" class="btn btn-danger">Details</button></div>`;
...
const updateTotal = () => {
    ...
    const grandTotal = (price + deliveryCharge + tax).toFixed(2);
    // update total
...
const loadProduct = product => {
    ...
    fetch(url)
        .then(res => res.json())
        .then(data => displayProductDetails(data[0]));
}
